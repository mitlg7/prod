...
values ('admin', '$2a$10$hUixmuP1Y0PqLpuqGwlY/.xuO.O1rDY8azyS7cCx05.fD2YRIZ5DO',
        (select id from role where type = 'ADMIN'));
insert into users (login, password, role_id)
values ('moder', '$2a$10$6ZEtqONqtv7OQ/Sr1ObOz.hLnVrkzqf8Z7baoCJ.vudKMd0OOnc72',
        (select id from role where type = 'MODER'));
insert into users (login, password, role_id)
values ('user', '$2a$10$AV3Ee0PERL7EqQnCodwGVuju6zJfExvPuHs1DvcdWLs294ZPUMzXy',
        (select id from role where type = 'USER'));
...
create procedure createUser(_login varchar(128), _password varchar(128), _email varchar(128), _token varchar(256))
insert into users (login, password, email, role_id, token)
values (_login, _password, _email, (SELECT ID FROM role WHERE type = 'USER'), _token);

create procedure deleteToken(_login varchar(128))
update users set token = null where login = _login;
...
create procedure createProduct(_user_id int, _image varchar(128), _name varchar(128), _description varchar(4096),
                               _price int, _date date, _cat_id int, _cntr int)
...
create procedure createUserInfo(_name varchar(128), _lastname varchar(128), _phone varchar(64), _image varchar(128),
                                _birthday date)
